...
<div class="container">
	...
	<?php
		...
		If(isset($_SESSION['login_user']))
		{
			$sql = "SELECT `student registration`.`username`,email,`books`.bid,name,authors,edition,issue,dueon FROM `student registration` inner join `issue-book` ON `student registration`.username=`issue-book`.username inner join books ON `issue-book`.bid=books.bid WHERE `issue-book`.approve='yes' ORDER BY `issue-book`.dueon ASC";
